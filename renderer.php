 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_pycode_renderer extends qtype_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa, question_display_options $options) {
        ...
        if (count($examples) > 0) {
            ...
            $qtext .= html_writer::start_tag('div', array('class' => 'pycode-examples'));
    ...
    protected function specific_feedback(question_attempt $qa) {
        ...
        if ($trSerialised) {
            ...
            if ($this->count_errors($testResults) == 0) {
                $resultsclass = "pycode-test-results-good";
            } else {
                $resultsclass = "pycode-test-results-bad";
            }
    ...
    private function buildResultsTable($testCases, $testResults) {
        ...
        // by looking for occurrences of shellinput and stdin data in the tests.
        
        list($numStdins, $numTests) = $this->countBits($testCases);
        
        $table = new html_table();
        $table->attributes['class'] = 'pycode-test-results';
        $table->head = array();
        if ($numTests) {
            $table->head[] = 'Test';
        }
        ...
        foreach ($testResults as $testResult) {
            if (!$testResult->hidden) {
                $tableRow = array();
                if (strpos($testResult->outcome, 'Error')) {
                    $result = $testResult->outcome . ': ' . $testResult->output;
                } else {
                    $result = $testResult->output;
                }
                if ($numTests) {
                    $tableRow[] = s($testResult->shellinput);
                }
                if ($numStdins) {
    ...
    private function formatExamplesOnePerLine($examples) {
       $text = '';
       foreach ($examples as $example) {
            $text .=  $example->shellinput . ' &rarr; ' . $example->output;
    ...
    private function formatExamplesAsTable($examples) {
        ...
        foreach ($examples as $example) {
            ...
            if ($numShell) {
                $row[] = $this->addLineBreaks(s($example->shellinput));
            }
